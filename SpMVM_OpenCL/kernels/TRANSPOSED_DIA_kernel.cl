...
__kernel void spmv_transposed_dia(__private unsigned int ndiags,
#if USE_CONSTANT_MEM
...
#pragma unroll
	for (i = 0; i < ndiags; i++)
	{
		...
		if (q >= 0 && q < N_MATRIX)
		{
			r += *(d_diags + diags_offset + i + row_id * ndiags) * d_x[q];
		}
	}
	dst_y[row_id] += r;
}
...
// OCCUPANCY TEST
__kernel void occ_spmv_transposed_dia(__private unsigned int ndiags,
#if USE_CONSTANT_MEM
	__constant int* d_ioff,
	__constant REAL* d_diags,
	__constant REAL* d_x,
