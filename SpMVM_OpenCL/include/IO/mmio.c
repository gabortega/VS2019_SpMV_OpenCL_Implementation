...
// Code was altered to work on Visual C++
...
int mm_read_banner(FILE *f, MM_typecode *matcode)
{
    ...
    if (sscanf_s(line, "%s %s %s %s %s", banner, (unsigned)_countof(banner), mtx, (unsigned)_countof(mtx) \
									, crd, (unsigned)_countof(crd), data_type, (unsigned)_countof(data_type) \
									, storage_scheme, (unsigned)_countof(storage_scheme)) != 5)
        return MM_PREMATURE_EOF;
...
char  *mm_typecode_to_str(MM_typecode matcode)
{
    ...
    sprintf_s(buffer, _countof(buffer),"%s %s %s %s", types[0], types[1], types[2], types[3]);
